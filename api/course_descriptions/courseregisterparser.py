...
from pdfminer.layout import LTText, LTChar, LTTextLine, LTContainer
...
class Course:
    ...
    def titlematch(self):
        titlematch = re.match("\s*(\d\d\d)\.\s*(\([^()]*\))?\s*(.*?)\.?\s*(\(.\))?\s*$", self.title)
        if not titlematch:
            print >> sys.stderr, "!!! REGEX ERROR:", self.title, "|", self.description
            return None
        else:
            ###num = self.title.split(".")[0]
            # num, crosslist, title, mysterycode
            return titlematch.groups()
...
def parseTextLine(textline):
    ...
    line = line.strip() + " "  # ("*\n\n" if endsection else " ")
    title = "".join(letter.text for letter in letters if letter.get_size() > 12)
