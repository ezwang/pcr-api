...
class APIObject:
  ...
  serializable object. Additional data (provided with add_data()) may be
  required for the encode() method; if this is not set, encode() will fail.
  However, encode_refr() should always work even if these parameters are not
  set."""
  ...
  def encode(self):
    """ Returns a JSON-serializible object representing the full state of the
    object. Throws APIObjectUninitializedError if not fully initialized. """
...
class APICourse(APIObject):
  ...
  def api_name(self):
    ...
    def capitalize(word):
      ...
      return word if word in roman_numerals else word.capitalize()
    return ' '.join([capitalize(w) for w in self.db_course.name.split(' ')]) 
  def api_url(self):
...
class APISection(APIObject):
  ...
  def api_id(self):
    # e.g. "12345-001"
    return "%s-%s" % (self.api_course.api_id(), self.sectionnum_str())
  def api_name(self):
    # "Programming Languages and Techniques I"
    return self.api_course.api_name()
  def api_url(self):
...
class APIInstructor(APIObject):
  ...
  def api_url(self):
    # temporary!
    return "/courses/instructor/%s/" % urllib.quote(self.api_name())
